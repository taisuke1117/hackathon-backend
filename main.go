...
func runMigrations(database *sql.DB) {
	statements := []struct {
		...
		// ── カテゴリ体系の更新（旧カテゴリを名前指定で削除） ──
		// 対応する新カテゴリはこの下の INSERT ループで再投入される
		{"旧カテゴリを削除（体系変更）", `DELETE FROM categories WHERE name IN (
			'キッズ・ベビー','アウター・ジャケット','バッグ・財布','シューズ',
			'アクセサリー・ジュエリー','家電・スマホ','PC・タブレット','カメラ・光学機器',
			'ゲーム・ホビー','おもちゃ・グッズ','スポーツ・アウトドア','インテリア・家具',
			'コスメ・美容・香水','食品・飲料','楽器・音響機器','ビンテージ・コレクション'
		)`},
	}
	...
	categories := []string{
		"レディース", "メンズ", "ベビー・キッズ",
		"ゲーム・おもちゃ・グッズ", "ホビー・楽器・アート",
		"本・雑誌・漫画", "CD・DVD・ブルーレイ",
		"スマホ・タブレット・パソコン", "テレビ・オーディオ・カメラ", "生活家電・空調",
		"スポーツ", "アウトドア・釣り・旅行用品",
		"コスメ・美容", "ダイエット・健康",
		"食品・飲料・酒", "キッチン・日用品・その他",
		"家具・インテリア", "ペット用品", "DIY・工具",
		"フラワー・ガーデニング", "ハンドメイド",
		"車・バイク・自転車", "チケット", "その他",
	}
	added := 0
